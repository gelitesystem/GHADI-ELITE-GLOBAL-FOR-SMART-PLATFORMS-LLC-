...
import { firestore, FIRESTORE_DATABASE_ID } from "../firebase";
import {
  LIMITS, type Approval, type AuditRecord, type EventRecord,
  type Health, type Locale, type PlanStep, type RunRecord,
  type ToolCategory, type Worker, redact, safeSummary
} from "../contracts";

const runs = firestore.collection("ghadiRuns");
const approvals = firestore.collection("ghadiApprovals");
...
function ar(locale: Locale) { return locale === "ar"; }
...
function inferIntent(request: string, locale: Locale) {
  if (/(analy[sz]e|analysis|تحليل|حلل)/i.test(request)) return ar(locale) ? "تحليل منظم للطلب" : "Structured request analysis";
  if (/(research|بحث|ابحث|مصادر)/i.test(request)) return ar(locale) ? "استكشاف مصادر ومعرفة" : "Research and knowledge exploration";
  if (/(build|create|develop|بناء|إنشاء|طور)/i.test(request)) return ar(locale) ? "تخطيط وتنفيذ بناء مضبوط" : "Controlled build planning and execution";
  return ar(locale) ? "تحويل النية إلى خطة تنفيذ مضبوطة" : "Transform intent into a controlled execution plan";
}

function workers(locale: Locale): Worker[] {
  return [
...
function makePlan(locale: Locale, sensitive: boolean): PlanStep[] {
  const plan: PlanStep[] = [
  ...
  if (sensitive) plan.push({ id: "approval", title: ar(locale) ? "تفويض حساس" : "Sensitive authorization", description: ar(locale) ? "يتطلب قرارًا بشريًا صريحًا." : "Requires an explicit human decision.", tool: "approval.gate", toolCategory: "high_risk", workerId: "critic", status: "waiting", requiresApproval: true });
  plan.push(
    { id: "verify", title: ar(locale) ? "التحقق" : "Verify", description: ar(locale) ? "تأكيد حدود الأدوات وعدم تجاوز السياسة." : "Confirm tool limits and policy.", tool: "guardrail.verify", toolCategory: "read", workerId: "critic", status: sensitive ? "pending" : "completed", requiresApproval: false },
    { id: "synthesize", title: ar(locale) ? "النتيجة" : "Result", description: ar(locale) ? "صياغة نتيجة لا تدعي أثرًا لم يحدث." : "Compose a result without claiming an effect that did not happen.", tool: "result.compose", toolCategory: "write", workerId: "conductor", status: sensitive ? "pending" : "completed", requiresApproval: false }
  );
  return plan.slice(0, LIMITS.maxSteps);
}

function event(runId: string, type: EventRecord["type"], message: string, severity: EventRecord["severity"]): EventRecord {
  return { id: id("evt"), at: isoNow(), type, message: redact(message), severity };
}

function audit(runId: string, category: AuditRecord["category"], action: string, outcome: AuditRecord["outcome"], details: string, toolCategory?: ToolCategory): AuditRecord {
  const record: AuditRecord = { id: id("audit"), at: isoNow(), category, action, outcome, runId, details: redact(details) };
  if (toolCategory) record.toolCategory = toolCategory;
  return record;
}

function geminiStatus(): Health["gemini"] {
  const key = process.env.GEMINI_API_KEY?.trim();
  if (!key) return "not_configured";
  return process.env.GHADI_ENABLE_GEMINI === "true" ? "enabled" : "configured_not_enabled";
}

async function optionalGemini(request: string, locale: Locale): Promise<string | undefined> {
  if (geminiStatus() !== "enabled") return undefined;
  const key = process.env.GEMINI_API_KEY?.trim();
  ...
  return response.text ? redact(response.text).slice(0, 1600) : undefined;
}

function finalResult(run: Partial<RunRecord> & { intent: string, locale: Locale, approvals: Approval[] }): string {
  ...
  const text = ar(run.locale)
    ? `أكمل Ghadi معالجة مضبوطة للنية «${run.intent}». حُفظت الحالة وسجل التدقيق في Firestore، وطُبقت حدود التنفيذ (${LIMITS.maxSteps} خطوات، ${LIMITS.maxToolCalls} استدعاء أدوات، ${LIMITS.maxRetries} إعادات). لم يُنفذ أي أثر خارجي غير مصرح به.${run.approvals.length ? " مر الإجراء الحساس عبر بوابة موافقة بشرية." : " لم يُرصد إجراء عالي الخطورة."}`
    : `Ghadi completed controlled processing for “${run.intent}”. State and audit were stored in Firestore, with limits of ${LIMITS.maxSteps} steps, ${LIMITS.maxToolCalls} tool calls, and ${LIMITS.maxRetries} retries. No unauthorized external effect was executed.${run.approvals.length ? " The sensitive action passed through human approval." : " No high-risk action was detected."}`;
  return text + suffix;
}

export class GhadiEngine {
  health(): Health {
    return { engine: "healthy", executionMode: "safe_server", stateStore: "firestore", firestoreDatabaseId: FIRESTORE_DATABASE_ID, gemini: geminiStatus(), rag: "not_configured", limits: LIMITS };
  }

  async submit(request: string, locale: Locale, idempotencyKey?: string): Promise<RunRecord> {
    const normalized = safeSummary(request);
    const runId = id("run");
    ...
    if (sensitive) {
      const approval: Approval = { id: id("approval"), runId, requestedAction: ar(locale) ? "أثر عالي الخطورة رُصد" : "High-risk effect detected", toolCategory: "high_risk", status: "pending", requestedAt: started };
    ...
    return firestore.runTransaction(async (tx) => {
      if (idempotencyKey) {
        ...
        if (!existing.empty) return existing.docs[0].data() as RunRecord;
      }
      tx.set(runs.doc(runId), { ...run, createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() });
      if (run.approvals[0]) tx.set(approvals.doc(run.approvals[0].id), { ...run.approvals[0], createdAt: FieldValue.serverTimestamp() });
      return run;
    });
  }
  ...
  async listRuns(limit = 50) {
    ...
    return snap.docs.map((doc) => doc.data() as RunRecord);
  }
  ...
  async decideApproval(approvalId: string, decision: "approved" | "rejected") {
    return firestore.runTransaction(async (tx) => {
      ...
      tx.set(runRef, { ...run, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
      return run;
    });
  }
}

export const ghadiEngine = new GhadiEngine();
